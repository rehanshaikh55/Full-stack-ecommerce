...
export const removeCartItem = (cart) => async (dispatch) => {
  dispatch({ type: REMOVE_CART_ITEM_REQUEST });

  try {
    const { data } = await api.delete(`/api/cart_items/${cart}`);
...
export const updateCartItem = (reqData) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({ type: UPDATE_CART_ITEM_FAILURE, error: error.message });
  }
};
